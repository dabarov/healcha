...
import { addDays, nowIso, todayLocal } from "@/lib/dates";
 ...
 * text-to-SQL can be explored without a Fitbit or Google Cloud setup.
 ...
 *
 * Refuses to run against a remote (libsql://) database unless force is set —
 * it's meant for the local file DB.
 */
...
async function seedIntraday(metas: DayMeta[], log: (msg: string) => void): Promise<void> {
...
export async function seedDemoData(
  ...
  if (!url.startsWith("file:") && !opts.force) {
    throw new Error(
      `Refusing to seed demo data into a remote DB (${url}). ` +
        `Use a local file database, or pass force if you really mean it.`,
    );
  }
  ...
  log(`Seeded ${DAYS} days of demo data; generating intraday…`);
  await seedIntraday(metas, log);
